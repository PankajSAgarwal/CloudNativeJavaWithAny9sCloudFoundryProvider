...
public class FinishedFileFlowConfiguration {
    ...
    IntegrationFlow finishedJobsFlow(BatchChannels channels,
                                     @Value("${complete-directory:${HOME}/Desktop/completed")File finished,
                                     JdbcTemplate jdbcTemplate){

        return IntegrationFlows.from(channels.completed())
                .handle(JobExecution.class, (je,headers)->{
                    ...
                    mv(file,finished);
                    List<Contact> contacts = jdbcTemplate.query("select * from contact", (rs, i) -> new Contact(
